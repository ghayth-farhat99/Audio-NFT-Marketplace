        ...
        address[] memory nftContracts = EnumerableSet.values(_nftContracts);
         ...
         for (uint i = 0; i < nftContracts.length; i++) {
            ...
            Listing storage listing = _listingMap[nftAddress][tokenId];
                ...
                if (listing.endDateUNIX < block.timestamp ) {
                // Update listing to initial state
                IERC4907(nftAddress).setUser(tokenId, address(0), 0);
                EnumerableSet.remove(_nftContractTokensMap[nftAddress], tokenId);
                delete _listingMap[nftAddress][tokenId];
                if (EnumerableSet.length(_nftContractTokensMap[nftAddress]) == 0) {
                    EnumerableSet.remove(_nftContracts, nftAddress);
                }
                _nftsListed.decrement();
                }
            }
